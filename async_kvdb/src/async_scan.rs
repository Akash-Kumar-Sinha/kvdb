use btree::{Node, PageId, PagerState, Value};
...
type Advanced<S> = (Cursor, Option<(S, Value)>);
...
fn advance<S>(pager_state: &Arc<SpinLock<PagerState>>, stack: &mut Cursor) -> Option<(S, Value)>
where
    S: Ord + Clone + Serialize + DeserializeOwned,
{
    loop {
        let (page_id, idx) = *stack.last()?;
        let node: Node<S> = pager_state
            .acquire()
            .pager
            .read_page(page_id)
            .expect("read failed");
        ...
        match step(&node, idx) {
            Step::Yield(i) => {
                cursor(stack);
                return Some((node.keys[i].clone(), node.values[i].clone()));
            }
