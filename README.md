...
A disk-backed, embedded key-value store built from scratch in Rust — a B-tree storage engine with a page-based on-disk format, a compile-time-enforced typestate API, and real thread-safe concurrency via a hand-rolled spinlock.
...
The public interface is `KvDb<S, LockState>` — `open` / `put` / `get` / `delete` / `range` / `len`, with `lock()`/`unlock()` enforcing at compile time when mutation is allowed. It's built on top of `BTree` (in the `btree` crate) and `SpinLock` (in the `spinlock` crate), but those are internal building blocks — `KvDb` is the intended entry point.
...
- **Rust's type system as a design tool** — typestate for compile-time state tracking, and a closed `Value` enum for compile-time-checked, self-describing storage instead of an opaque generic.
...
`Pager` is wrapped in a `SpinLock<T>` (its own crate, `spinlock`) — `AtomicBool`-based, `compare_exchange` for acquire, a `store` for release, `UnsafeCell<T>` holding the guarded data, and an RAII guard whose `Drop` releases the lock automatically. `unsafe impl Send`/`Sync` is written and justified explicitly, not assumed.

`BTree`/`KvDb` hold this behind an `Arc`, so multiple handles — potentially on different threads — can safely share the same underlying storage. Every public entry point (`put`/`get`/`delete`/`range`/`len`) acquires the lock exactly once per call and passes the already-unlocked guard down through internal helpers, rather than each helper acquiring independently — an earlier version that acquired per-helper deadlocked reliably on any operation that triggered a B-tree split, since a function would try to re-acquire a lock its own caller already held.
...
`KvDb` is the intended way to use this project. `BTree` and `SpinLock` are internal building blocks it's composed from, not separate public-facing interfaces.
...
- Locking is coarse-grained (one `SpinLock` guards the whole `Pager`, not true per-page "crabbing"), and the concurrent test suite currently covers multi-reader/single-writer only. `root_id` is still cached per-`KvDb`/`BTree` clone rather than living inside the shared lock, so genuine multi-_writer_ safety under concurrent splits isn't proven yet.
